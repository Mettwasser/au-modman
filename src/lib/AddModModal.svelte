<script lang="ts">
	...
	async function createMod() {
		...
		if (!/\d+.\d+.\d+/gm.test(version)) {
			const toast: ToastSettings = {
				message: 'Please use the Version format "major.minor.patch" e.g. "1.1.0"',
				background: 'variant-filled-error',
				timeout: 5000
			};
			toastStore.trigger(toast);
		...
		confirmed = true;
		await invoke('add_modification', { name: profileName, version, downloadUrl: modUrl }).catch((why) => {
			const toast: ToastSettings = {
...
{#if $modalStore[0]}
	<div class="max-h-[650px] w-[650px] rounded-lg bg-surface-800">
		<div class="flex flex-col justify-start space-y-2 p-8">
			<h2 class="mb-4 ml-4 text-center text-4xl">Add a Mod</h2>
			<div
				class="
			...
			>
				...
				<div>
					<label for="modUrlInputField">Mod Download URL</label>
					<input
						id="modUrlInputField"
						autocomplete="off"
						bind:value={modUrl}
				...
				<div>
					<label for="versionInputField">Version</label>
					<input
			...
			<div class="mr-5 flex justify-end">
				...
				{#if !confirmed}
					<!-- LOCK SCREEN -->
					<button class="variant-filled-success btn !text-white outline-0" on:click={createMod}>Create</button>
				{:else}
